...
def process_audio_queue():
    """Process and speak items in the audio queue, discarding outdated messages"""
    global stop_audio_thread

    # Maximum age for messages in seconds
    MAX_MESSAGE_AGE = 5.0
    ...
    while not stop_audio_thread:
        try:
            ...
            try:
                priority, timestamp, text = audio_queue.get(block=True, timeout=1.0)

                # Check if the message is too old
                current_time = time.time()
                if current_time - timestamp > MAX_MESSAGE_AGE:
                    print(f"Discarding outdated message (age: {current_time - timestamp:.2f}s): {text}")
